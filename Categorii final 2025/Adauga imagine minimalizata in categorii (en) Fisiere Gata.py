...
def get_image_from_article_page(url, priority_folder, backup_folder):
    """Extrage imaginea din pagină - caută mai întâi local, apoi online"""
    try:
        ...
        if img_src and img_alt:
            print(f"    Găsit în folder prioritar: {img_src}")
            return img_src, img_alt

        # Caută în folderul de backup
        backup_path = os.path.join(backup_folder, file_name)
        print(f"    Caut în folder backup: {backup_path}")
        img_src, img_alt = get_image_from_local_file(backup_path)

        if img_src and img_alt:
            print(f"    Găsit în folder backup: {img_src}")
            return img_src, img_alt

        # Ultimă opțiune - descarcă de pe web
        print(f"    Nu găsit local, încerc online: {url}")
        headers = {
        ...
        if feature_img:
            img = feature_img.find('img')
            if img and img.get('src'):
                print(f"    Găsit online: {img.get('src')}")
                return img.get('src'), img.get('alt', '')
            ...
            if iframe and 'youtube.com/embed/' in str(iframe.get('src', '')):
                ...
                video_id = iframe_src.split('/embed/')[-1].split('?')[0]
                thumbnail_url = f"https://img.youtube.com/vi/{video_id}/maxresdefault.jpg"
                print(f"    Găsit video YouTube: {thumbnail_url}")
                return thumbnail_url, iframe.get('title', 'Video YouTube')
        ...
        for img in soup.find_all('img'):
            src = img.get('src', '')
            if 'images/' in src and not src.endswith('.gif'):
                print(f"    Găsit fallback online: {src}")
                return src, img.get('alt', '')

        return None, None

    except Exception as e:
        print(f"Eroare la extragerea imaginii pentru {url}: {e}")
        return None, None

def process_html_file(file_path, priority_folder, backup_folder, base_url="https://neculaifantanaru.com"):
    """Procesează fișierul HTML cu căutare în două foldere"""
    ...
    try:
        ...
        for i, article in enumerate(articles):
            ...
            try:
                ...
                # Extrage imaginea cu căutare în două foldere
                img_src, img_alt = get_image_from_article_page(full_url, priority_folder, backup_folder)
    ...
    <div class="blog-listing-inner news_item">
        <div class="article-card-new">
            ...
            <div class="mobile-layout d-block d-md-none">
                ...
                time.sleep(0.2)  # Pauză mai mică pentru fișiere locale
...
def main():
    ...
    # Căile folderelor
    priority_folder = r"c:\Folder1\fisiere_gata"
    backup_folder = r"e:\Carte\BB\17 - Site Leadership\Principal 2022\en"

    print("Începe procesarea cu căutare în două foldere...")
    print(f"Folder prioritar: {priority_folder}")
    ...
    for file_name in category_files:
        ...
        if os.path.exists(file_path):
            process_html_file(file_path, priority_folder, backup_folder)
            processed_files += 1
        else:
            print(f"- Nu există în folder prioritar: {file_name}")
        print("-" * 30)

    print(f"Finalizat! Procesate {processed_files} fișiere.")
